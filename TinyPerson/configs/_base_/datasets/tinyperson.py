...
data = dict(
    ...
    train=dict(
        type=dataset_type,
        ann_file=data_root + 'ssp_anno/train/train_overlap30.json',
        img_prefix=data_root + 'train',
        pipeline=train_pipeline),
    val=dict(
        type=dataset_type,
        ann_file=data_root + 'ssp_anno/test/tiny_set_test_single.json',
    ...
    test=dict(
        ...
        img_prefix=data_root + 'test',
        pipeline=test_pipeline))
evaluation = dict(interval=5, metric='bbox', proposal_nums=[1000], iou_thrs=[0.5])
