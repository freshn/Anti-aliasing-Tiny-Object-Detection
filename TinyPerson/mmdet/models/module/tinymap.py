...
class Heatmap:
    def __init__(self, fpn_lvl=4,
        ...
        self.lamda = 0.0001  # balanced parameter of reg loss
    ...
    def reg_mask(self, pred_att, gt_att):
        ...
        selected_mask[(gt_att < 0.0)] = self.neg_weight
        #selected_mask[(gt_att > 0.0)] = gt_att[gt_att > 0.0]
        return selected_mask
    ...
    def target_single(self, anns, lvl, img_h, img_w):
        ...
        pt_x = np.arange(1, img_w + 1)[np.newaxis, :].repeat(img_h, axis=0)
        pt_y = np.arange(1, img_h + 1)[:, np.newaxis].repeat(img_w, axis=1)
        tblr_target = np.ones((img_h, img_w, 4))
        for ann in anns:
            x1, y1, x2, y2 = ann
            ...
            b = int(np.clip(np.ceil(y2), a_min=0, a_max=img_h - 1))
            ...
            tblr_target[t:b, l:r, 0] = pt_x[t:b, l:r] - l  # left
            tblr_target[t:b, l:r, 1] = pt_y[t:b, l:r] - t  # top
            tblr_target[t:b, l:r, 2] = r + 1 - pt_x[t:b, l:r]  # right
            tblr_target[t:b, l:r, 3] = b + 1 - pt_y[t:b, l:r]  # bottom
            if value == 1:
                gt_mp[t:b, l:r] = self.centerness(tblr_target[t:b, l:r].reshape(-1, 4)).reshape(h, w)
            #else:
            #    gt_mp[t:b, l:r] = -1
    ...
    def centerness(self, pos_bbox):
        left_right = pos_bbox[:, [0, 2]]
        top_bottom = pos_bbox[:, [1, 3]]
        if len(left_right) == 0:
            centerness_targets = left_right[..., 0]
        else:
            centerness_targets = (left_right.min(axis=-1) / left_right.max(axis=-1)) \
                                 * (top_bottom.min(axis=-1) / top_bottom.max(axis=-1))
        return np.sqrt(centerness_targets)
