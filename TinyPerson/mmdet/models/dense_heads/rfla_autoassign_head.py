...
from mmdet.core import distance2bbox, multi_apply, build_assigner
...
class CenterPrior(nn.Module):
    ...
    def __init__(self,
        ...
        self.mean = nn.Parameter(torch.zeros(num_classes, 2)) # initialize the mean matrix
        self.sigma = nn.Parameter(torch.ones(num_classes, 2)) # initialize the sigma matrix
    ...
    def forward(self, anchor_points_list, gt_bboxes, labels,
        ...
        for slvl_points, stride in zip(anchor_points_list, self.strides):
            ...
            instance_center = self.mean[labels][None] #why
            # instance_sigma has shape (1, num_gt, 2)
            instance_sigma = self.sigma[labels][None] #why
...
class RFLA_AutoAssignHead(FCOSHead):
    ...
    def loss(self,
        ...
        all_num_gt = sum([len(item) for item in gt_bboxes]) #number of gts
        ...
        for gt_bboxe, gt_label, inside_gt_bbox_mask in zip(
            ...
            temp_inside_gt_bbox_mask_list.append(inside_gt_bbox_mask) #get the center prior weight
    ...
    def _get_target_single(self, gt_bboxes, points, rfields):
        ...
        if num_gts:
            ...
            inside_gt_bbox_mask = inds[...,None].repeat(1, num_gts)
        else:
